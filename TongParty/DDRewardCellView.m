...
-(void)setupViews{
    ...
    self.iconView.image = [UIImage imageNamed:@"usercenter_newgift"];
...
- (void)updateWithModel:(DDUserInfoModel *)model{
    if (![DDUserDefault objectForKey:@"token"]){
        ...
        if (model.be_coin) {
            self.coinLabel.text = model.be_coin;
        }
    }
}
...
-(void)setType:(DDRewardType)type{
    switch (type) {
        ...
        case DDRewardTypeHis:
        {
            ...
            [self.accessBtn setBackgroundImage:kImage(@"btnbg_image") forState:UIControlStateNormal];
            [self.accessBtn addTarget:self action:@selector(playReward:) forControlEvents:UIControlEventTouchUpInside];
            [self.accessBtn setTitleColor:kWhiteColor forState:UIControlStateNormal];
