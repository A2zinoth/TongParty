...
- (UITableView *)tableview{
    if (!_tableview) {
        self.tableview = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight- kTabBarHeight - kNavigationBarHeight) style:UITableViewStyleGrouped];
...
- (UIButton *)btn_createDesk {
    ...
    if (!_btn_createDesk) {
        ...
        _btn_createDesk.backgroundColor = kBgBlueColor;
...
//                    weakSelf.requestEntity.latitude = [NSString stringWithFormat:@"%lf",POI.location.latitude];
    ...
    if (indexPath.section == 2) {
        switch (indexPath.row) {
            case 1:{
                LSTimeRangePicker* _picker = [[LSTimeRangePicker alloc]init];
                [_picker setContentMode:DDPickerContentModeBottom];
                _picker.timeRangeData = ^(NSString * _Nonnull day, NSString * _Nonnull hour, NSString * _Nonnull min, NSString * _Nonnull seconds) {
                    cell.tf_action_content.text = [NSString stringWithFormat:@"%@%@%@",day,hour,min];
                    _requestEntity.time_range = seconds;
                };
                [_picker  show];
            }break;
