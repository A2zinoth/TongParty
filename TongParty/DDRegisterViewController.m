...
- (UIButton *)regiterBtn{
    if (!_regiterBtn) {
        ...
        _regiterBtn.backgroundColor = kBgBlueColor;
...
//        _messageLabel.text = @"登录后即表示同意桐聚《用户协议》";
        ...
        [str addAttributes:@{
                             NSForegroundColorAttributeName:kBgBlueColor
                             } range:strRange];
...
-(void)registerAction{
    ...
    if ([self.validateManager validateLoginWithModel:model]) {
        [DDTJHttpRequest registerWithMobile:model.username passwd:model.password code:model.vercode block:^(NSDictionary *dict) {
            [DDTJHttpRequest loginWithMobile:self.phoneTxt.textFiled.text password:self.psdTxt.textFiled.text block:^(NSDictionary *dict) {
                [DDUserDefault setObject:self.phoneTxt.textFiled.text forKey:@"mobile"];
                [DDUserDefault setObject:self.psdTxt.textFiled.text forKey:@"password"];
