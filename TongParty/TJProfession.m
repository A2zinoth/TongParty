...
- (NSString *)getSelectTheme {
    for (int i = 0; i < 5; i++) {
        ...
        if(otherBtn.selected == true) {
            //aid string
            TJProfessionModel *model = _dataArr[[self getKind]];
            TJProfessionModel *submodel = model.child[i];
            
            return submodel.career_name;
        }
    }
    return @"1";
}
...
- (void)updateRightBtn:(NSInteger )index {
    TJProfessionModel *model = self.dataArr[index];
    ...
    for (int j = 0; j < 16; j++) {
        ...
        if (j < submode.count) {
            TJProfessionModel *model = submode[j];
            [rightBtn setTitle:model.career_name forState:UIControlStateNormal];
        } else {
...
- (void)updateData:(NSArray *)arr {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        if (i<arr.count) {
            ...
            TJProfessionModel *model = arr[i];
            [otherBtn setTitle:model.career_name forState:UIControlStateNormal];
            [self updateRightBtn:0];
        } else {
