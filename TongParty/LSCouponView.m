...
- (void)showCouponViewOnWindowWithType:(LSCouponViewShowType)type doneBlock:(void (^)(NSDictionary *dict))doneDict{
    if (type == DDDeskShowTypeJoinCoupon) {
        ...
        [self.couponView addSubview:_btn_confirm];
        [_btn_confirm mas_makeConstraints:^(MASConstraintMaker *make) {
            make.right.equalTo(self.couponView.mas_centerX).offset(-DDFitWidth(18.f));
            make.bottom.mas_equalTo(DDFitHeight(-15.f));
            make.height.mas_equalTo(DDFitHeight(30.f));
            make.width.equalTo(self.couponView).multipliedBy(0.3f);
        }];
        _btn_confirm.titleLabel.font = DDFitFont(15.f);
        ...
        [_btn_couponConfirm mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(self.couponView.mas_centerX).offset(DDFitWidth(18.f));
            make.bottom.mas_equalTo(DDFitHeight(-15.f));
            make.height.mas_equalTo(DDFitHeight(30.f));
            make.width.equalTo(self.couponView).multipliedBy(0.3f);
        }];
        ...
        _btn_confirm.layer.borderColor = kBgBlueColor.CGColor;
        _btn_confirm.layer.borderWidth = 1.f;
        _btn_confirm.backgroundColor = kWhiteColor;
        [_btn_confirm setTitleColor:kBgBlueColor forState:UIControlStateNormal];
        _btn_confirm.layer.cornerRadius = 15.f;
        ...
        _btn_couponConfirm.backgroundColor = kBgBlueColor;
        [_btn_couponConfirm setTitleColor:kWhiteColor forState:UIControlStateNormal];
        _btn_couponConfirm.layer.cornerRadius = 15.f;
    }
}
...
- (void)addCoupon:(UIButton *)sender {
    ...
    _lbl_count.text = [NSString stringWithFormat:@"%ld",couponCount*5];
    [_reslutDict setObject:@(couponCount) forKey:@"couponCount"];
