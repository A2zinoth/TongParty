...
//    [animateView CrentBtnImageName:@"pop_joinlovedesk" Title:@"加入心跳桌" tag:2];
    //Add center button
    [animateView CrentCenterBtnImageName:@"tabbar_center_close" tag:3];
    //Do animation
...
- (instancetype)init{
    ...
    if (self)
    {
        ...
        [visualEffectView setFrame:CGRectMake(0, self.bounds.size.height - DDFitHeight(280.f), self.bounds.size.width, DDFitHeight(280.f))];
...
- (void)CrentBtnImageName:(NSString *)ImageName Title:(NSString *)Title tag:(int)tag{
    ...
    rect.size = CGSizeMake((rect.size.width-10)*1.8f, (rect.size.height-10)*1.8f);
...
- (void)BtnClick:(UIButton*)btn{
    [kNotificationCenter postNotificationName:@"centerPopAction" object:btn];
    [self.delegate didSelectBtnWithBtnTag:btn.tag];
    [self removeFromSuperview];
}
...
//            btn.center = CGPointMake((74+i++*113)*bl, self.frame.size.height-165*bl);
            btn.center = CGPointMake((95+i++*190)*bl, self.frame.size.height-165*bl);
        } completion:nil];
