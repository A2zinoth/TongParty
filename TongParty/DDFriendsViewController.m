...
- (DDCustomCommonEmptyView *)emptyView {
    if (!_emptyView) {
        DDCustomCommonEmptyView *empty = [[DDCustomCommonEmptyView alloc] initWithTitle:@"" secondTitle:@"暂无数据" iconname:@"noData"];
    ...
    switch (_style) {
        case DDFriendsStyleNormal: {
            if (_type == DDFriendCurrentUserStyle) {
                ...
                // 他人朋友列表
                [DDTJHttpRequest getFriendsListWithFid:_fid block:^(NSDictionary *dict) {
                    [self hideLoadingAnimation];
                    self.dataArray = [LSFriendEntity mj_objectArrayWithKeyValuesArray:dict];
                    if (_dataArray.count == 0) {
                        [self.emptyView showInView:self.view];
        ...
        case DDFriendsStyleCare:{
            if (_type == DDFriendCurrentUserStyle) {
                ...
                [DDTJHttpRequest getOhterCareListByFid:_fid block:^(NSDictionary *dict) {
                    [self hideLoadingAnimation];
                    self.dataArray = [LSCareEntity mj_objectArrayWithKeyValuesArray:dict];
                    if (_dataArray.count == 0) {
                        [self.emptyView showInView:self.view];
